...
public class MainPageVm : ViewModelBase, IDisposable
{
    ...
    readonly Command _saveCmd;
    readonly Command _requestUrlCmd;
    readonly Command _cancelUrlCmd;
    CancellationTokenSource? _request_cts;

    public MainPageVm()
    {
        _saveCmd = new Command(async () =>
        {
            if (Url is null)
        ...
        _requestUrlCmd = new Command(async () =>
        {
            ...
            try
            {
                ...
                foreach (var h in response.Content.Headers)
                {
                    ...
                    {
                        var contentTypeValue = h.Value.First();
                        if (contentTypeValue.StartsWith("video/"))
                        {
                            var subtype = contentTypeValue.Split('/').ElementAtOrDefault(1)?.Split(';')[0].Trim() ?? "mp4";
                            _contentType = ContentType.Video;
                            byte[] bytes = await response.Content.ReadAsByteArrayAsync(_request_cts.Token).CAF();
                            _tmpFilePath = StaticValues.CurrentSaveVideoFilePath(subtype);
                        ...
                        {
                            var subtype = contentTypeValue.Split('/').ElementAtOrDefault(1)?.Split(';')[0].Trim() ?? "jpg";
        ...
        _cancelUrlCmd = new Command(async () =>
        {
    ...
    public bool VideoVis => _contentType == ContentType.Video;
    public bool ImageVis => _contentType == ContentType.Image;
    public bool TextVis  => _contentType == ContentType.Text;
    ...
    public Command SaveCmd => _saveCmd;
    public Command RequestUrlCmd => _requestUrlCmd;
    public Command CancelUrlCmd => _cancelUrlCmd;
